...
serialize_to_builder_any :: proc(a: any, sb: ^strings.Builder, indentation: int = 0, config: Serializer_Config) {
	...
	#partial switch info in ti.variant {
		...
		case reflect.Type_Info_Procedure, reflect.Type_Info_Pointer:
			// TODO: This function itself should write its `key`, so in a case like this we just don't
			//       write anything, because firstly we don't need to write and also writing nil is
...
contains_tag :: proc(struct_tags: reflect.Struct_Tag, tag_name: string) -> (ok: bool, err: mem.Allocator_Error) {
	if len(struct_tags) == 0 do return false, nil
